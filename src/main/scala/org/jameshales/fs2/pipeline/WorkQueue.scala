...
import fs2.{ Chunk, NonEmptyChunk, Pipe, Stream }
import fs2.async.immutable
import fs2.async.mutable
import fs2.async.mutable.Queue
import fs2.util.{ Applicative, Async, Functor, Traverse }
...
trait WorkQueue[F[_], A, B] { self =>
  ...
  def dequeueBatch: Pipe[F, Int, B] =
    _.flatMap { batchSize =>
      Stream.bracket(cancellableDequeueBatch1(batchSize))(
        d => Stream.eval(d._1).flatMap(Stream.chunk),
        d => d._2
      )
    }

  def dequeueAvailable: Stream[F, B] =
    Stream.constant(Int.MaxValue).through(dequeueBatch)
  ...
  def map[C](f: B => C)(implicit F: Functor[F]): WorkQueue[F, A, C] =
    new WorkQueue[F, A, C] {
      ...
      override def dequeueBatch1(batchSize: Int): F[NonEmptyChunk[C]] =
...
object WorkQueue {
  implicit val TraverseNonEmptyChunk: Traverse[NonEmptyChunk] =
